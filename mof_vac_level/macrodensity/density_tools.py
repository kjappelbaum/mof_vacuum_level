...
import numba as nb
import numpy as np
from numba import NumbaPendingDeprecationWarning, jit
...
def cube_potential(origin, travelled, cube, Grid, nx, ny, nz):
    ...
    travelled_i, travelled_j, travelled_k = travelled[0], travelled[1], travelled[2]
    ...
    potential_cube = np.zeros(shape=(i, j, k))

    for x in range(0, i):
        for y in range(0, j):
            for z in range(0, k):
                # Assign the values of coordinates in the original grid
                xv = int(n_origin[0] + travelled_i + x)
                yv = int(n_origin[1] + travelled_j + y)
                zv = int(n_origin[2] + travelled_k + z)
    ...
    mean = potential_cube.mean()
    var = np.var(potential_cube)
    return mean, var
